...
| D-r3f-dataset-tranche1 | Primeiros exemplos de dataset 3D (React Three Fiber) usando o backend recém-construído (`D-checker-node-backend`), tranche 1 de um conjunto maior baseado no briefing de 24 prompts do usuário. 5 exemplos (`gen-r3f-*.json`), todos com `write_file`/`checker(language="typescript", operation="run")` reais — `run` builda com `vite build` de verdade e renderiza num Chromium headless real (WebGL), `build_example` levanta `AssertionError` se qualquer passo falhar: (1) `hero-abstract-sculpture` — hero de IA com escultura abstrata (núcleo icosaédrico metálico + 3 anéis torus emissivos + campo de partículas em casca esférica), reação ao cursor amortecida por lerp (sem giro exagerado), `prefers-reduced-motion` num hook próprio, densidade de partículas 400→120 em telas < 768px; (2) `neural-network-instanced` — rede neural 3D com nós e pulsos em InstancedMesh (não centenas de componentes React), conexões num único lineSegments, pulsos viajando pelas arestas via mutação de matriz no useFrame (nunca setState por frame), painel HTML sincronizado com a cena por um único estado `selected`; (3) `city-instanced-seed` — cidade miniatura de 300 prédios em InstancedMesh gerada por PRNG de seed fixa (mulberry32, determinística), veículos animados por ref, névoa; (4) `fix-low-performance` (`task_type: model_fixes_after_error`) — projeto quebrado seedado (400 `<mesh>` separados + setState no useFrame) que o agente lê, diagnostica os dois antipadrões e refatora pra 1 InstancedMesh com mutação direta de matriz; (5) `validate-real-ops-only` (`task_type: checker_infra_unavailable`) — ataca DIRETAMENTE o achado real do checkpoint-228 (inventar `render_and_validate`): o agente valida só com `syntax_check`+`run` reais e declara explicitamente no `<final>` que o checker NÃO inspeciona aparência, sem fingir uma operação de validação visual | Usuário pediu pra começar a gerar o dataset 3D depois do backend estar pronto e testado; confirmou que lembrava dos 24 prompts e que o checker funcionava | **Bug real de asset remoto encontrado e corrigido durante a geração** (o checker fez seu trabalho): a primeira versão do hero usava `<Environment preset="city" />` do drei, que baixa um HDR remoto (`potsdamer_platz_1k.hdr` de um CDN) — o checker offline falhou com `net::ERR_NAME_NOT_RESOLVED`/`Failed to fetch`, exatamente a violação do requisito "não depender de assets externos" que o briefing pede evitar; trocado por luzes locais (hemisphere+directional+2 point lights). **Segundo ajuste, de estética** (checker não pega, screenshot pega): o núcleo com `metalness=1` sem environment map renderizava quase preto (armadilha conhecida do three.js — metais refletem o ambiente, sem envmap só mostram specular); ajustado pra `metalness=0.6`/base mais clara + luzes mais fortes, confirmado por screenshot real que passou a ler como metal iluminado. Screenshot real capturado e inspecionado visualmente na conversa pros 4 exemplos visuais (hero, rede neural com painel sincronizado, cidade com névoa/veículos, octaedro flat-shaded) — todos com composição forte e sem asset externo. Validação estrutural (`structural_validate`/`validate_metadata`) 0 problemas nos 5; 0 ids duplicados (2547 total). Suite Python completa: 202/202 (mesma falha pré-existente do `trl`). Falta: tranches 2/3 cobrindo os prompts restantes do briefing (produto 3D com hotspots, card holográfico, galeria, planeta, portal, CFD, configurador, shaders, correções de vazamento/mobile/asset-glb, e os de constraint: sem-R3F/sem-esferas/tema-claro/editorial/sem-loop/WebGPU) |
| D-r3f-dataset-tranche2 | Tranche 2 do dataset 3D, mais 5 exemplos (`gen-r3f-*`/`gen-three-vanilla-*.json`), todos com `write_file`/`checker(run)` reais (vite build + Chromium render): (#2) `product-headphone-hotspots` — fone premium composto só de primitivas (meio-torus pro arco, cilindros pras conchas/almofadas), flutuando, girável por OrbitControls, 3 hotspots via `<Html>` do drei (DOM real ancorado a posição 3D, continua legível com a câmera girando, não vira textura); (#5) `planet-procedural` — planeta com continentes gerados numa `CanvasTexture` LOCAL (document.createElement, sem fetch remoto), atmosfera (casca BackSide), nuvens translúcidas, terminador dia/noite por luz direcional, cidades (pontos por lat/lon) ligadas por arcos de Bézier; (#17) `recover-missing-glb` (`model_fixes_after_error`) — projeto seedado com `useGLTF("/model.glb")` pra um asset inexistente; o agente lê, LISTA o projeto pra confirmar a ausência, e substitui por composição procedural preservando a API (`<ProductModel/>` com ponto marcado pra reintroduzir o GLB) + `<Suspense>`, sem inventar o arquivo nem manter URL quebrada; (#19) `three-vanilla-no-r3f` — hero em Three.js PURO, sem R3F/drei, estrutura modular (scene.ts com dispose, main.ts com renderer/resize/rAF/cleanup completo), o agente traz o próprio index.html + main.ts; (#20) `no-spheres-architectural` — composição só de formas angulares (grade de pilares retangulares, tema claro, sombras reais), atacando o clichê "3D = esfera brilhante" | Usuário pediu pra continuar do tranche 1 ("Pode fazer") | **Melhoria de infra necessária pro exemplo vanilla (#19)**: o scaffold do backend (`node_backend.py`) copiava `src/main.tsx` do template sempre que o agente não fornecia — mas um projeto Three.js vanilla traz seu próprio `index.html` + `src/main.ts` e NÃO um `App.tsx`, então o `main.tsx` órfão do template importava um `./App` inexistente e quebrava o `tsc`. Refatorado: config (`vite.config`/`tsconfig`/`package.json`) continua sempre copiada; o par de bootstrap (`index.html` + `src/main.tsx`) só é copiado quando o agente NÃO traz o próprio entry point (nenhum `index.html`/`src/main.*`) — se traz, ele bootstrapa à própria maneira. Os 7 testes de `test_checker_node.py` continuam passando (caso comum R3F: agente traz só `App.tsx` → bootstrap copiado → funciona). Screenshot real dos 4 exemplos visuais inspecionado na conversa (fone reconhecível com hotspots, planeta com continentes/atmosfera/dia-noite/arcos, torus knot metálico vanilla, composição arquitetônica de tema claro com sombras). Validação estrutural 0 problemas; 0 ids duplicados (2552 total). Suite Python completa: 202/202 (mesma falha pré-existente do `trl`). Falta a tranche 3: card holográfico, galeria espacial, portal, CFD, configurador, os 3 de shader (material líquido/campo de energia/dissolução), correções de vazamento-de-memória/mobile, e os constraints restantes (tema-claro dedicado, editorial, sem-loop-de-animação, WebGPU-com-fallback) |
| D-r3f-dataset-tranche3 | Tranche 3 do dataset 3D, mais 6 exemplos, todos com `write_file`/`checker(run)` reais (vite build + Chromium render): (#7) `cfd-flow-field` — visualização de escoamento sobre base FÍSICA real (solução analítica do escoamento potencial 2D ao redor de um cilindro, isolada numa função `velocity(x,y,U)` — o ponto de troca pra dados reais depois), linhas de corrente integradas por Euler, 500 partículas advectadas em InstancedMesh coloridas pela velocidade, legenda e 3 presets HTML; screenshot confirmou que a física lê certo (partículas aceleram/aquecem sobre topo/base do cilindro, estagnam/esfriam na frente/trás); (#9) `product-configurator` — tênis estilizado de caixas, 3 partes colorizáveis + acabamento + reset, com o ponto técnico central sendo a DISCIPLINA DE ESTADO (config é estado React que só muda em ação do usuário, useFrame só gira por ref, nenhum setState por frame — exatamente o antipadrão que o briefing e a doc do R3F alertam); (#23) `on-demand-rendering` — `frameloop="demand"`, renderiza só em interação/resize/mudança de estado (invalidate manual via componente Invalidator), sem nenhum useFrame; (#15) `fix-resource-leak` (`model_fixes_after_error`) — projeto seedado com 3 vazamentos reais (geometria/textura criadas por render sem dispose, addEventListener sem remoção, setInterval sem clear), o agente lê, diagnostica os três e corrige o ciclo de vida (useMemo + useEffect cleanup) preservando o design; (#16) `fix-mobile-compat` (`model_fixes_after_error`) — projeto seedado com canvas de tamanho FIXO em px, dpr={3}, 8000 partículas, sem touch-action nem reduced-motion; o agente corrige os 5 (canvas responsivo, dpr clamped, densidade adaptativa, touch-action, frameloop demand em reduced-motion) com degradação gráfica controlada; (#11) `liquid-shader-fallback` — esfera líquida com shader GLSL PRÓPRIO (vertex desloca por onda senoidal 3D animada ao longo das normais, fragment colore por normal + Fresnel), uniforms controlados por sliders HTML, e fallback via React Error Boundary trocando pra meshPhysicalMaterial se o shader falhar — o único exemplo de shader do dataset (dose controlada, como o briefing pede) | Usuário pediu pra continuar ("Quero sim. E commite e push") | Todos os 6 passaram build+render real; o exemplo de shader (#11) confirma que o checker valida GLSL de verdade (o shader compilou sem erro de console do WebGL no Chromium headless). Screenshot real inspecionado na conversa dos 4 principais (CFD com física legível, configurador com painel completo, shader líquido distorcido com Fresnel, sob-demanda) — o CFD e o shader líquido saíram visualmente fortes; o configurador ficou funcional mas "blocky" (aceito: o briefing permite representação estilizada, e o valor pedagógico é a disciplina de estado, não o realismo do tênis). Validação estrutural 0 problemas; 0 ids duplicados (2558 total). Suite Python completa: 202/202 (mesma falha pré-existente do `trl`). Cobertura do briefing de 24 agora: #1,2,5,7,8,9,10,11,14,15,16,17,18,19,20,23 (16/24). Falta uma tranche 4: card holográfico (#3), galeria espacial (#4), portal (#6), campo de energia (#12), dissolução (#13), tema-claro dedicado (#21), editorial (#22), WebGPU-com-fallback (#24) |
| D-checker-scss-backend | Primeiro passo do plano de expansão web (o usuário pediu um plano técnico pra deixar o modelo forte em frontend/Node/JS/CSS/SCSS, e depois: "não quero ficar preso ao scss, faça com CSS também"). Insight de design que resolve o "não ficar preso": **CSS puro JÁ é capacidade de primeira classe** — o backend `html` (`D-checker-html-backend`) renderiza `.css` real no Chromium (é o que os 31 exemplos elegantes já fazem), nenhuma infra nova precisa. O SCSS é o que faltava, e foi construído pra COMPARTILHAR o oráculo visual do CSS: novo `src/checker/backends/scss_backend.py`, registrado sob `"scss"`, compila `.scss → .css` com dart-sass de verdade (pacote `sass` adicionado ao `package.json` do template Node, reaproveitando a junção NTFS de `node_modules` de `D-checker-node-backend` — sem `npm install` por chamada) e, na operação `run`, passa o CSS compilado pelo MESMO `render_and_capture` (Chromium headless) que o backend `html` usa. Assim `.css` valida direto via `language: "html"`, `.scss` compila-e-valida via `language: "scss"`, os dois terminam renderizados no navegador real — nunca preso a um. Operações: `compile`/`syntax_check` = compilação dart-sass (erro real → `COMPILATION_ERROR`); `run` = compila + renderiza (precisa de um `.html` que linke o `.css` gerado, senão `INCOMPLETE_SOLUTION` honesto); `lint` = `MISSING_DEPENDENCY` (stylelint não implementado, mesmo padrão honesto dos outros backends) | Usuário pediu explicitamente ("faça com CSS também") depois de eu recomendar começar a expansão web pelo backend SCSS; a decisão de compartilhar o oráculo CSS/SCSS resolve o "não ficar preso" de forma estrutural, não só cosmética | Testado de ponta a ponta ANTES dos testes automatizados com um SCSS real (variáveis + `@mixin pill` + nesting + `&:hover`): `compile` passou em ~0,7s, `run` compilou e renderizou em ~2,3s, screenshot real do card resultante inspecionado visualmente na conversa (mixin/nesting/variáveis todos aplicados corretamente no navegador). Confirmado que erro real de SCSS (variável indefinida) vira `COMPILATION_ERROR`, e que CSS puro segue validando pelo backend `html`. 6 testes novos (`tests/unit/test_checker_scss.py`, mesmo padrão skip-condicional): compile passa/falha, run renderiza, run exige `.html`, sem `.scss` é `UNSUPPORTED_LANGUAGE`, `"scss"` registrado. `MVP_LANGUAGES` (`taxonomy.py`) e o enum de `language` em `src/schemas/checker.json` ganharam `"scss"`. Suite Python completa: 208/208 (202 + 6 novos, mesma falha pré-existente do `trl`). Próximo: dataset de CSS/SCSS profundo (layout/grid/responsivo/animações + nesting/mixins/funções), validado por compile+render real — pode começar já que a infra existe |
| D-css-scss-dataset-fase-b | Fase B do plano de expansão web (`plano técnico para deixar o modelo forte em frontend/Node/JS/CSS/SCSS`): primeiro lote de dataset CSS/SCSS profundo, 10 exemplos, todos com `write_file`/`checker` reais (CSS via `language: "html"`, SCSS via `language: "scss"`, compartilhando o oráculo visual do Chromium). 5 CSS puro: `grid-dashboard-areas` (grid-template-areas nomeadas, breakpoint só redeclara o mapa); `flexbox-wrap-cards` (flex-wrap + flex-basis, quebra sem media query decidindo o corte); `container-queries-card` (container-type/@container — o MESMO componente reage à largura do container, não da viewport, dois exemplos lado a lado na mesma página provando isso); `animations-skeleton-button` (transition pra mudança de estado simples vs. @keyframes pra loop contínuo, os dois desligados sob prefers-reduced-motion); `fix-fixed-footer-overlap` (`model_fixes_after_error` — position:fixed tirando o footer do fluxo e cobrindo o hero, corrigido sem gambiarra de padding-bottom). 5 SCSS: `nesting-bem-card` (nesting com `&` espelhando hierarquia BEM); `mixins-functions-tokens` (`@function` de escala de espaçamento, `@mixin` com `@content` pra breakpoint reutilizável, mixin com parâmetro default); `use-partials-modules` (`_tokens.scss`+`_mixins.scss` importados via `@use` namespaceado em `main.scss`, não o `@import` depreciado); `maps-each-utilities` (SCSS maps + `@each` gerando classes utilitárias de margin/padding/cor sem escrever nenhuma à mão); `fix-missing-include` (`model_fixes_after_error` — chamada de mixin sem `@include`, o agente roda o `checker` DE VERDADE pra confirmar o sintoma antes de corrigir, não assume) | Usuário disse "Faça" depois de eu recomendar começar a Fase B com o backend SCSS recém-pronto | Todos os 10 passaram de primeira (nenhum erro/retry precisou de correção nesta leva). Screenshot real de 4 amostras inspecionado na conversa: grid de dashboard renderizou as áreas nomeadas corretamente, os mixins/função/breakpoint SCSS geraram os dois botões com cor e padding certos, e o `@each` sobre maps gerou exatamente as classes esperadas (`.p-4`/`.bg-primary`/`.text-success`/`.m-0`) sem nenhuma escrita manual. Validação estrutural 0 problemas; 0 ids duplicados (2568 total). Suite Python completa: 208/208 (mesma falha pré-existente do `trl`). Dataset frontend/JS agora ~88 exemplos (CSS/SCSS 10 + 3D R3F 16 + elegante Apple 31 + explorar-antes-de-editar parcial). Falta: mais rodadas de CSS/SCSS (Fase B contínua), JS/TS+DOM (Fase C), React profundo (Fase D), Node backend (Fase E, ainda sem infra), correções multi-camada (Fase F) |
| D-css-scss-dataset-fase-b-lote2 | Fase B, segundo lote — 9 exemplos cobrindo território não tocado no lote 1: CSS `grid-masonry-dense` (grid-auto-flow: dense preenchendo buracos automaticamente, spans de coluna/linha); `keyframes-toast-lifecycle` (@keyframes de 4 paradas percentuais — entra/fica/sai numa timeline só, fill-mode forwards, sem JS controlando estados); `custom-properties-theme-toggle` (tema claro/escuro: prefers-color-scheme como padrão, [data-theme] de maior especificidade sobrepondo quando o usuário força); `scroll-driven-progress-bar` (animation-timeline: scroll(root), feature CSS nativa recente, sem scroll listener JS, com fallback @supports); `fix-specificity-active-link` (`model_fixes_after_error` — ID (1,1,1) vencendo classe dupla (0,2,1) mesmo vindo depois no arquivo, corrigido sem !important igualando a especificidade). SCSS: `if-else-contrast-text` (função com @if/@else + color.lightness() decidindo cor de texto legível automaticamente a partir do fundo); `for-stagger-animation` (@for gerando 6 animation-delays escalonados a partir de 1 valor base); `placeholder-extend-buttons` (%placeholder + @extend agrupando CSS de saída sem duplicar propriedades nem virar classe própria no HTML); `fix-extend-across-media-query` (`hypothesis_revision_after_failure` — achado real ao gerar: a correção óbvia, `@extend` de um placeholder externo DE DENTRO de uma media query, é rejeitada pelo próprio dart-sass com erro real "You may not @extend selectors across media queries"; o agente testa essa hipótese primeiro via `checker` de verdade, vê o erro real, e corrige extendendo numa regra de nível superior fora da media query) | Usuário pediu "Faça o 1 primeiro" (mais uma rodada de CSS/SCSS) | **Achado real durante a geração, não hipotético**: o autor deste lote (a sessão) errou a primeira hipótese sobre o bug de `@extend`/media query — achou que seria só "esqueceram o @extend", tentou adicionar `@extend %card-base` dentro da media query como fix, e essa tentativa REALMENTE falhou ao compilar (erro genuíno do dart-sass sobre não poder estender através de fronteiras de media query). Em vez de descartar o achado, o exemplo foi reestruturado pra ensinar exatamente essa sequência real (tentativa → falha real → correção certa), e o `task_type` mudou de `model_fixes_after_error` pra `hypothesis_revision_after_failure` pra refletir isso com precisão. Screenshot real de 4 amostras inspecionado — masonry preenchendo buracos corretamente (span largo encaixando sem espaço vazio), contraste @if/@else escolhendo texto escuro/branco certo nos 4 fundos. Validação estrutural 0 problemas; 0 ids duplicados (2577 total). Suite Python completa: 208/208 (mesma falha pré-existente do `trl`). Dataset frontend/JS agora ~97 exemplos |
| D-checker-node-server-backend | Fecha a Fase A (infra) do plano de expansão web: Node server-side + `eslint` (`lint`), os dois últimos backends que faltavam. Usuário pediu "vá fazendo em ordem" depois de eu listar as fases restantes. **Node server-side**: novo `src/checker/backends/node_server_backend.py`, registrado sob `"node"` — DISTINTO de `"javascript"`/`"typescript"` (`node_backend.py`, orientados a React/Vite/browser, sem sentido pra scripts server-side). `run` executa o script via `node` de verdade (stdout/stderr/exit code reais — cobre CLIs); `compile_and_test` reaproveita `vitest` (já existia) — a chave é que testar um servidor Express não precisa de bind de porta nem subprocess separado: `supertest` (adicionado ao template) envolve o app Express e despacha requisições HTTP REAIS pelo pipeline de rotas/middleware de verdade, sem mock, é o jeito idiomático de testar Express em produção — então "subir servidor, bater endpoint" já funciona só de reaproveitar o `vitest` existente. **`eslint` real** (`lint`, antes `MISSING_DEPENDENCY` em todos os backends JS/TS/node): nova função compartilhada `lint()` em `node_backend.py`, usada tanto por ele quanto por `node_server_backend.py`, com `eslint.config.js` novo no template (`eslint:recommended` + `typescript-eslint` recommended, regras de correção não de estilo) | Usuário pediu pra seguir em ordem pelo plano listado; a ordem colocava Fase A (infra) antes de continuar C/D/E/F | **Três bugs reais encontrados e corrigidos rodando testes de ponta a ponta antes de qualquer teste automatizado, nenhum hipotético**: (1) `eslint.config.js` sem `languageOptions.globals` fazia `console`/`process`/etc. serem marcados como `no-undef` — falso positivo que tornaria o linter inútil; corrigido com o pacote `globals` (`...globals.node, ...globals.browser`). (2) `_sanitize_base_dir` (função compartilhada de TODOS os backends, D-checker-path-leak) remove a string do diretório temp de qualquer stdout/stderr — ao remover o prefixo absoluto de dentro do valor JSON `"filePath"` do relatório do eslint, sobra uma barra invertida solta que corrompe o JSON inteiro (`Invalid \escape`); corrigido escrevendo o relatório num ARQUIVO (`--output-file`, fora do pipeline de sanitização de stdout) e nunca expondo o `filePath` absoluto do eslint — os erros são remapeados pro path RELATIVO original (`zip(targets, reports)`, ordem preservada pelo eslint), eliminando o vazamento na origem em vez de tentar filtrar depois. (3) Sem `package.json` copiado no sandbox do `lint`, o Node não achava `"type":"module"` localmente e subia a árvore de diretórios REAL da máquina até achar um `package.json` de outra pasta qualquer do usuário — vazando esse path num aviso; corrigido copiando `package.json` do template junto com `eslint.config.js`. **Quarto bug, mais sério, achado testando o Express+supertest**: `run_command` (`_subprocess_utils.py`, compartilhado por TODOS os backends — Python/Go/HTML/SCSS/Node) chamava `subprocess.run(..., text=True)` SEM `encoding` explícito — no Windows isso usa o codepage do console (`cp1252` nesta máquina), que não decodifica bytes UTF-8 de caracteres acentuados (ex.: "obrigatório" numa mensagem de erro de teste); a thread leitora do subprocess morria com `UnicodeDecodeError` silenciosamente, corrompendo o resultado (`'NoneType' object has no attribute 'encode'`). Corrigido adicionando `encoding="utf-8", errors="replace"` explícitos — afeta TODOS os backends que usam `run_command`, não só os novos, e é a classe de bug mais perigosa desta entrada (silenciosa, dependente de conteúdo específico do texto, só apareceu com uma palavra acentuada real num teste). 18 testes novos (`tests/unit/test_checker_node_server.py` + adições em `test_checker_node.py`): CLI real via `node`, erro de runtime real capturado, servidor Express testado via `supertest` (passa E falha corretamente quando o endpoint quebra), `lint` pegando bug real (`no-undef`/`no-cond-assign`) sem falso positivo em `console`, teste dedicado de não-vazamento de path, `MISSING_DEPENDENCY` estruturado, `"node"` registrado como linguagem distinta. `MVP_LANGUAGES`/schema/taxonomy atualizados com `"node"`. Suite Python completa: 219/219 (208 + 11 novos líquidos, mesma falha pré-existente do `trl`, sem regressão em nenhum backend existente) |
| D-js-ts-dom-dataset-fase-c | Fase C do plano de expansão web (JS/TS + DOM), primeiro lote — 8 exemplos, todos com `write_file`/`checker` reais (DOM via `language: "html"` no Chromium; lógica pura via `language: "node"` com `vitest` real). DOM/eventos: `dom-todo-list` (createElement em vez de innerHTML+string, pra evitar injeção; fonte única de verdade array→render); `event-delegation-dynamic-list` (um listener no pai via bubbling, cobre itens criados depois, `closest('li')`); `debounce-stale-response-guard` (debounce clássico + contador de `requestId` pra descartar respostas assíncronas obsoletas — race condition real de UI, não hipotética). Lógica pura: `closures-counter-memoize` (closures isoladas por instância + memoização com `Map`, testado com `vi.fn()` spy confirmando que a função original só roda uma vez por combinação de args); `ts-generic-stack-balanced` (Stack<T> genérica aplicada a verificação de parênteses balanceados); `ts-result-type-no-exceptions` (union discriminada Result<T,E> pra erro esperado sem exceção, com `mapResult` genérico). Correções: `fix-var-loop-closure-bug` (`var` em loop compartilhando uma única variável entre todos os callbacks de `setTimeout` — corrigido pra `let`, escopo de bloco); `fix-stale-closure-click-count` (closure capturando uma cópia do estado no momento da criação do listener, não lendo o valor atual no momento do clique) | Usuário pediu pra continuar em ordem pelo plano ("faça, meu caro sócio") | **Quatro bugs reais encontrados e corrigidos no PRÓPRIO backend `node_server_backend.py` (não no dataset), rodando os exemplos de TypeScript antes de aceitar qualquer coisa como pronta**: (1) `tsc --noEmit --allowJs` sem tsconfig.json E sem arquivos passados explicitamente não sabe o que compilar — imprime a TELA DE AJUDA (não um erro) e sai com código 1, que o backend interpretava erroneamente como `COMPILATION_ERROR`; corrigido passando os `.ts`/`.tsx` explicitamente na linha de comando. (2) faltava `--allowImportingTsExtensions`, então `import ... from './stack.ts'` (extensão explícita, prática comum em ESM real) falhava com `TS5097`. (3) **achado mais sutil**: sem `--strict` (desligado por padrão sem tsconfig), o estreitamento de tipo por NEGAÇÃO booleana (`if (!result.ok)`) numa union discriminada não funcionava — só `=== false` estreitava corretamente; reproduzido isoladamente nos dois sentidos (com e sem `--strict`) antes de decidir a flag, não assumido. `--strict` adicionado, bate com o `tsconfig.json` do template de qualquer forma. (4) o exemplo `ts-result-type-no-exceptions` também expôs um limite real de inferência do TypeScript com genéricos dentro de ternário (`result.ok ? ok(fn(result.value)) : result` falhava a checagem de tipo entre `Result<T,E>` e `Result<U,E>` mesmo sendo estruturalmente compatível em runtime) — contornado reescrevendo `mapResult` com `if/return` explícito em vez de ternário, registrado no próprio `<think>` do exemplo como lição real, não só corrigido silenciosamente. Interação real testada via Playwright direto (não só `render_and_capture` estático): todo list clicado de verdade (adicionar 2 itens, marcar 1 concluído) — contador e riscado corretos na screenshot. Validação estrutural 0 problemas; 0 ids duplicados (2585 total). Suite Python completa: 219/219 (mesma falha pré-existente do `trl`, sem regressão) |
| D-react-deep-dataset-fase-d-lote1 | Fase D do plano de expansão web (React profundo), primeiro lote — 8 exemplos, todos com `write_file`/`checker(language="typescript", operation="run")` reais (build Vite + render Chromium real). Hooks/estado: `hook-uselocalstorage` (hook customizado com leitura lazy do `useState` pra ler o storage só uma vez, escrita num `useEffect`, os dois lados tolerando `localStorage` indisponível em try/catch sem quebrar a UI); `context-reducer-cart` (Context + `useReducer` pra um carrinho — ADD soma quantidade se já existe, `SET_QUANTITY` remove automaticamente ao chegar a zero, `useCart()` lança erro explícito fora do provider). Formulário/dados: `controlled-form-validation` (padrão touched — erro só aparece depois do `onBlur`, submit marca tudo como touched pra revelar erros em campos nunca visitados); `data-fetching-states` (union discriminada de estado loading/error/success tornando combinações inválidas irrepresentáveis por construção, mais uma flag `cancelled` fechada por closure no cleanup do `useEffect` pra nunca fazer `setState` após desmontagem). Performance: `usememo-usecallback-perf` (dois problemas distintos na mesma tela — `useMemo` evita recalcular um total custoso quando só um contador não relacionado muda, `useCallback` mantém identidade estável de uma função passada a um filho `memo()`, sem o que a memoização do filho seria inútil). Acessibilidade: `accessible-dropdown-keyboard` (padrão ARIA Listbox completo — `aria-haspopup`/`aria-expanded`/`role=listbox`/`aria-activedescendant`, navegação por setas com `preventDefault`, Enter/Espaço confirma, Esc cancela, foco sempre devolvido ao botão). Correções (`model_fixes_after_error`): `fix-stale-closure-useeffect` (array de dependências vazio prendia o `count` lido dentro de um `setInterval` no valor da montagem — corrigido declarando `count` como dependência, forçando o efeito a recriar o interval com o valor atual); `fix-unnecessary-rerender-object-prop` (objeto literal recriado a cada render invalidava um `React.memo` por comparação rasa de referência — corrigido com `useMemo` mantendo a mesma referência entre renders, sem remover o memo do filho) | Usuário confirmou com "FAÇA" a continuação proposta pro próximo passo da ordem do plano (Fase D — React profundo) | Todos os 8 passaram de primeira, cada um buildado e renderizado de verdade no Chromium headless (não apenas type-checado) — inclusive os dois exemplos de correção, cujo `checker` final roda sobre a versão CORRIGIDA do arquivo, provando que o fix compila e monta sem erro de console, não só "parece certo" na leitura. Validação estrutural 0 problemas; 0 ids duplicados (2593 total). Suite Python completa: 219/219 (mesma falha pré-existente do `trl`, sem regressão) |
| D-3d-tranche4-briefing-final | Fecha o briefing original de 24 prompts 3D (tranches 1-3 já feitas em sessões anteriores) — os 8 exemplos remanescentes, todos com `write_file`/`checker(language="typescript", operation="run")` reais, só luzes locais (nunca HDR remoto). `#3` `holographic-card-shader` (shader customizado com termo de Fresnel + conversão HSL→RGB manual pro efeito de cor mudando com ângulo de visão); `#4` `space-gallery-navigable` (câmera fixa no centro de um círculo de peças, rig de navegação manual); `#6` `interactive-portal-mesh` (`MeshPortalMaterial` do drei — cena interna real via stencil buffer, não textura, clique troca o tema); `#12` `energy-field-shader` (FBM de 4 oitavas sobre ruído de valor por hash, UV deslocada por tempo em duas direções); `#13` `dissolve-transition-shader` (`discard` condicional a ruído quantizado por posição + borda incandescente via smoothstep estreito, threshold oscilando com o tempo); `#21` `premium-light-theme-showcase` (fundo bege claro dedicado, ContactShadows + Float do drei, tema claro que NENHUM exemplo anterior tinha como estética principal); `#22` `minimalist-editorial-layout` (tipografia serifada confinada a 38% da largura, acento 3D deliberadamente mínimo pra não competir com o texto); `#24` `webgpu-detection-webgl-fallback` (`'gpu' in navigator` — detecção honesta, nunca alega renderizar via WebGPU quando o renderer real é sempre WebGL do R3F) | Usuário confirmou com "FAÇA" a continuação proposta (Fase D + tranche 4 dos 3D); esta entrega fecha a tranche 4 | **Três bugs reais de composição de cena encontrados só por inspeção visual de screenshot — nenhum causava erro de console, então o `checker` automatizado (que só detecta erro de build/runtime) os aprovaria como "passou" apesar da cena estar visualmente quebrada, um limite real do oráculo automatizado pra este domínio, não hipotético.** (1) `space-gallery-navigable`: a primeira versão usava `OrbitControls` pra girar a visão, mas `OrbitControls` ORBITA a câmera AO REDOR de um alvo e sempre aponta PRA ELE — com a câmera já perto da origem (o alvo padrão), ela girava num raio minúsculo sempre olhando pro próprio centro, nunca pra fora, pro círculo de peças a 4 unidades — screenshot mostrava tela vazia. Corrigido com um rig de navegação manual: a câmera fica fixa na origem, um grupo gira continuamente por tempo (mais arraste horizontal do usuário), e a câmera recalcula seu `lookAt` a cada frame na direção do grupo rotacionado. (2) Mesmo exemplo, segundo bug encadeado: depois de corrigir a câmera, a peça aparecia mas com o texto ESPELHADO — a normal da face frontal do `planeGeometry` das peças apontava pra FORA do círculo (mesma direção da posição), não pra dentro, então a câmera via o VERSO da peça; corrigido rotacionando por `angle + Math.PI` em vez de só `angle`. (3) `holographic-card-shader`: card totalmente de frente pra câmera (sem interação) renderizava como cor sólida — o termo de Fresnel fica quase uniforme quando a normal é paralela à direção de visão em TODA a superfície visível, escondendo o efeito principal do pedido numa screenshot automatizada sem mouse real se movendo; corrigido com oscilação-base contínua por tempo somada à reação ao ponteiro. Todos os 3 fixes reconfirmados renderizando de novo e inspecionando a nova screenshot antes de aceitar como prontos — não bastou "sem erro de console" pra nenhum dos três. Validação estrutural 0 problemas; 0 ids duplicados (2601 total). Suite Python completa: 219/219 (mesma falha pré-existente do `trl`, sem regressão) |
| D-node-backend-multiturn-fase-e | Fase E do plano de expansão web (Node backend) — e, junto, o pedido explícito do usuário de "fluxos de conversas multi turno criando projeto, lendo os arquivos, etc". 6 exemplos, cada um com DOIS turnos reais executados na MESMA `SandboxContext` (não sandboxes separadas) — turno 1 cria um projeto pequeno (vira `HistoryTurn` via `Trajectory.history`, D-dataset-history-schema/D-dataset-history-loss-mask, o mesmo mecanismo já usado por `gen-mt-late-*`/`gen-mt-abrupt-*`, mas ali com fillers genéricos sem ferramenta — aqui os DOIS turnos rodam ferramentas de verdade no mesmo sandbox, então o turno 2 realmente encontra e lê os arquivos que o turno 1 escreveu). `add-delete-endpoint` (turno 1: API Express de tarefas com GET/POST; turno 2: `read_file` + adiciona DELETE /tasks/:id com 404, testado com supertest); `cli-sum-multiple-files` (turno 1: `countLines`; turno 2: lê o arquivo, reaproveita a função existente em vez de duplicar lógica, adiciona `countMultipleFiles` agregando por `Map`); `fix-cpf-formatted-input-bug` (turno 1: validador de CPF com algoritmo real de módulo 11, aceita só dígitos puros; turno 2, `task_type: debugging`: usuário reporta que CPF formatado com pontos/traço é rejeitado — o agente ESCREVE um teste de reprodução e roda o checker sobre o código NÃO corrigido primeiro, confirma a falha real, só then corrige removendo não-dígitos no início da função, roda de novo confirmando fix); `add-logging-middleware` (turno 1: Express com /health; turno 2: middleware de log registrado ANTES das rotas, guardado num array exportado testável em vez de só `console.log`); `read-only-explain-csv-parser` (`task_type: code_explanation`, turno 2 só `read_file` + `<final>`, SEM chamada de checker — convenção já estabelecida no dataset pra esse task_type, `tools_used: ["read_file"]`, confirmada checando outros exemplos `code_explanation` existentes antes de decidir); `add-login-validation-security` (turno 1: endpoint de login mockado, sempre sucesso, pedido explicitamente como mock pra desbloquear frontend; turno 2: usuário pede validação real de campos obrigatórios por preocupação de segurança, agente concorda e adiciona validação de presença com 400 + mensagem específica, sem quebrar o teste de sucesso original) | Usuário pediu explicitamente pra continuar "a próxima fase, inclusive de fluxos de conversas multi turno criando projeto, lendo os arquivos, etc" | Bug real de convenção encontrado ANTES de gerar tudo: o helper genérico `_run_turn` inicialmente exigia que o ÚLTIMO passo de QUALQUER turno fosse um `checker` — isso quebraria a convenção já estabelecida pra `code_explanation` (`tools_used: ["read_file"]`, sem checker, confirmado grepando exemplos `aug-explain-go-*` existentes antes de assumir); corrigido com um parâmetro `require_checker_last=False` pro turno de explicação, mantendo os outros 5 exemplos (que de fato editam/estendem código) exigindo checker real no fim. Verificação end-to-end própria do multi-turno: reconstruí a `Trajectory` a partir do JSON salvo e chamei `render_for_model()` de verdade, confirmando que o `history` produz exatamente 2 ocorrências de `[USER]`/`[ASSISTANT]` no prompt renderizado, byte a byte no formato que `D-dataset-history-loss-mask` já mascara corretamente — não assumido, checado. No exemplo de bug de CPF, confirmei via regex sobre o `raw_text` salvo que a sequência de status dos dois `<tool_result name="checker">` do turno 2 é `["error", "ok"]` — a reprodução falhou de verdade antes da correção, não é uma falha narrada sem lastro. Validação estrutural 0 problemas; 0 ids duplicados (2607 total). Suite Python completa: 219/219 (mesma falha pré-existente do `trl`, sem regressão) |
